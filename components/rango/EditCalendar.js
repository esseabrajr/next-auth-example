...
const EditCalendar = ({
  ...
  const getEventBadge = (date, view) => {
    const selected = events.filter((line) => sameDay(line.date,date)).map((line) => new Date(line.date).getHours())
    return (view === 'month' && selected.length > 0 ? <EventDot selected={selected}/> : <div style={{width: "15px", height: "15px", backgroundColor: "transparent"}}></div>)
  }
